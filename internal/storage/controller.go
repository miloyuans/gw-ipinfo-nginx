...
type Replayer interface {
	Name() string
	Replay(ctx context.Context, client *mongostore.Client, batchSize int) error
}
...
func (c *Controller) HandleMongoError(err error) {
	...
	if c.degraded.CompareAndSwap(false, true) {
		c.mode.Store(ModeLocal)
		if c.logger != nil {
			c.logger.Warn("mongo_degraded_to_local",
		...
		case <-probeTicker.C:
			if c.degraded.Load() {
				if err := c.recoverAndReplay(ctx); err != nil && c.logger != nil {
					c.logger.Warn("mongo_replay_error",
						"event", "mongo_replay_error",
						"error", err,
					)
				}
			}
...
func (c *Controller) recoverAndReplay(ctx context.Context) error {
	...
	if c.logger != nil {
		c.logger.Info("mongo_recovered_replaying_local",
	...
	for _, replayer := range replayers {
		if err := replayer.Replay(ctx, client, c.cfg.ReplayBatchSize); err != nil {
			return errors.Join(errors.New(replayer.Name()), err)
		}
	}
	...
	c.mode.Store(ModeHybrid)
	if c.logger != nil {
		c.logger.Info("mongo_replay_done",
